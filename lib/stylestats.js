...
StyleStats.prototype.getDataUriSize = function() {
    var string = '';
    this.declarations.forEach(function(declaration) {
        if (declaration.value.indexOf('data:image') > -1) {
            string += declaration.value.match(/data\:image\/[A-Za-z0-9;,\+\=\/]+/);
        }
    });
...
StyleStats.prototype.parse = function(callback) {
    ...
    async.eachSeries(this.urls, function iterator(url, next) {
        ...
        var identifierCount = that.getIdentifierCount();
        identifierCount.sort(function(a, b) {
            return b.count - a.count;
        });

        var dataUriSize = that.getDataUriSize();
        ...
        if (options.dataUriSize) {
            stats.dataUriSize = dataUriSize;
        }
        if (options.dataUriSize && options.raitoOfDataUriSize && dataUriSize !== 0) {
            stats.raitoOfDataUriSize = dataUriSize / that.cssSize;
        }
        ...
        var mostIdentifer = identifierCount.shift();
