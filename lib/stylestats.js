...
StyleStats.prototype.parse = function(callback) {
    ...
    Promise.all(requestPromises).done(function onFulfilled(value) {
        ...
        if (options.dataUriSize && options.raitoOfDataUriSize && declarationAnalysis.dataUriSize !== 0) {
            stats.raitoOfDataUriSize = declarationAnalysis.dataUriSize / that.cssSize;
        }
        ...
        if (mostIdentifier && options.mostIdentifers) {
            stats.mostIdentifers = mostIdentifier.count;
        }
        if (mostIdentifier && options.mostIdentifersSelector) {
            stats.mostIdentifersSelector = mostIdentifier.selector;
        }
