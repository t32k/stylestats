...
Analyzer.prototype.analyzeDeclarations = function() {
    ...
    this.declarations.forEach(function(declaration) {
        ...
        if (declaration.property.indexOf('font-family') > -1) {
            var formatedFontNamesStr = declaration.value.replace(/(\!important|"|')/g, '').trim();
            formatedFontNamesStr.split(',').forEach(function(fontName) {
                result.uniqueFontFamily.push(fontName);
            });
        }
