...
class Analyzer {
  ...
  analyzeSelectors() {
    ...
    this.selectors.forEach(function (selector) {
      ...
      // if it contains * and dose not contain * in attribute selector
      if (selector.indexOf('*') > -1) {
        let universal = selector.replace(/\[.+\]/g, '');
  ...
  analyzeDeclarations() {
    ...
    this.declarations.forEach(function (declaration) {
      ...
      if (declaration.property.indexOf('background') > -1 && declaration.value.indexOf('url') > -1) {
        let paths = declaration.value.match(/url\(([^)]+)\)/g);
        if (paths) {
    ...
    result.uniqueFontFamilies = result.uniqueFontFamilies.filter(function (fontFamily, index, array) {
      return array.indexOf(fontFamily) === index;
    }).sort();

    // Sort `font-size` property.
    result.uniqueFontSizes = result.uniqueFontSizes.filter(function (fontSize, index, array) {
    ...
    let uniqueFontSizes = {};
    result.uniqueFontSizes.forEach(function (value) {
    ...
    Object.keys(uniqueFontSizes).forEach(function (key) {
      uniqueFontSizes[key].forEach(function (value) {
        result.uniqueFontSizes.push(value);
      });
    });

    // Sort `color` property.
    let trimmedColors = result.uniqueColors.filter(function (uniqueColor) {
    ...
    result.uniqueColors = formattedColors.filter(function (formattedColor, index, array) {
      return array.indexOf(formattedColor) === index;
    }).sort();

    // if it contains background-image url()
    result.uniqueBackgroundImages = result.uniqueBackgroundImages.filter(function (backgroundImage, index, array) {
  ...
  analyze() {
    // get analytics
    let ruleAnalysis = this.analyzeRules();
    let selectorAnalysis = this.analyzeSelectors();
    let declarationAnalysis = this.analyzeDeclarations();
    ...
    if (this.options.simplicity) {
      let simplicity = analysis.rules / this.selectors.length;
    ...
    // Most Identifier
    let mostIdentifier = selectorAnalysis.identifiers.shift();
    ...
    let lowestDefinition = ruleAnalysis.cssDeclarations.shift();
