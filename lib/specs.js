...
function buildTest(metric, spec, actual, defaults) {
  ...
  // text
  let defaultText = '{metric}: {actual} should be {operation} {expected}';
...
function testRunner(results, specs) {
  let testSpecs = parseSpecs(specs);
  let reporter = testSpecs.defaults.reporter || 'spec';
  let defaults = testSpecs.defaults || {};
  let tests = [];
  let path = [];
  ...
  // Mocha
  let mocha = new Mocha({reporter: reporter});
